...
@player_blueprint.route('/deleteAthleteRow/<email>/<qID>')
def deleteAthlete(email, qID):
    cursor = mysql.connection.cursor()
    cursor.execute("DELETE FROM emails_verify WHERE email = '%s' AND questionnaire_id = '%s'"%(email, qID))
    mysql.connection.commit()
