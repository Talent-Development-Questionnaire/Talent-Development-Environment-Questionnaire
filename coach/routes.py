...
def addAthletes(athlete, email, otp, quiz_name):
    cursor =  mysql.connection.cursor()
    cursor.execute("INSERT INTO emails_verify (questionnaire_id, email, code) VALUES ((SELECT questionnaire_id FROM questionnaire WHERE name = '%s' and coach_id = (SELECT coach_id FROM coaches WHERE email = '%s')), '%s', '%s')" % (quiz_$
    mysql.connection.commit()
